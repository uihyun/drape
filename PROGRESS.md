...
A web deploy is not a release. Anything user-facing needs a native build behind
it.
...
try-on "fits" economy (5/day + invite +10, Phase 1 = manual code; Phase 2 auto-
attribution + IAP still TODO), public-profile tab auto-hide + notch parity,
admin boards column, focus-crop fix
...
- **Image A/B rigs are kept** at `functions/test-item-crop-ab.js` (garment crop)
...
- **In-app notification center on the profile bell (2026-07-02) — SHIPPED in 1.3.0 (released 2026-07-05).**
  ...
  Push + bell are **unified** in `functions/notifications.js` (every bell row also
  pushes; the separate like/try-on push in `social-push.js` was removed to avoid
  double-fire) and **localized** per recipient via `profiles/{uid}.lang` — social
  push AND the DM image placeholder. Server writes/push are **live for everyone
  now**; the bell UI + deep-link routing land natively in **1.3.0**. Full record in
  CHANGELOG 1.3.0.
