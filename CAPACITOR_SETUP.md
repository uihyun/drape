...
- [x] `@capacitor/push-notifications@^7` 의존성 추가
- [x] `src/services/push-service.js` — Capacitor 네이티브에서만 register, 토큰을 `users/{uid}/fcmTokens/{token}` 서브컬렉션에 저장. 권한 거부 / 웹 환경 안전하게 no-op
- [x] `App.jsx` — 로그인 시 `PushService.ensureRegistered()` 호출, 로그아웃 시 토큰 제거
- [x] `firestore.rules` — `users/{uid}/fcmTokens/{token}` 자가 쓰기만 허용, 클라이언트 read 차단 (admin SDK 만 fan-out)
- [x] `functions/messages.js` — `onMessageCreated` 트리거. threads/{tid}/messages/{mid} 생성 시:
  1. 부모 thread 의 `activeIn[recipientUid] === true` 이면 푸시 스킵 (실시간 클라이언트로 이미 전달됨)
  2. 보낸 사람 프로필에서 displayName/handle 가져와 알림 title
  3. recipient 의 fcmTokens 가져와 `sendEachForMulticast`
  4. `messaging/registration-token-not-registered` 등 dead token 자동 prune
  5. iOS APNs `thread-id` + Android `collapseKey` 로 같은 스레드 알림 묶음
- [x] Notification tap → `data.threadId` payload → `/messages/{tid}` 딥링크 (push-service 의 `pushNotificationActionPerformed` 리스너)

#### 사용자 작업 (남음 — 키 없으면 푸시 실제 발송 안 됨)

##### Firebase Console
- [ ] **Project settings → Cloud Messaging** 탭 열기
- [ ] **Cloud Messaging API (V1)** 이 enabled 상태 확인 (기본 enabled)

##### iOS — APNs 키
- [ ] Apple Developer Console → Certificates, Identifiers & Profiles → **Keys** → "+"
- [ ] **Apple Push Notifications service (APNs)** 체크 → Continue → Register
- [ ] `.p8` 파일 다운로드 + Key ID 메모
- [ ] Firebase Console → Project settings → Cloud Messaging → **iOS app configuration → APNs Authentication Key** → Upload
  - Key ID: 위에서 받은 값
  - Team ID: `WG75TG59NJ`
- [ ] Xcode 열기 (`npx cap open ios`) → Target → Signing & Capabilities → "+" → **Push Notifications** 추가
- [ ] **Background Modes** capability 도 추가 → "Remote notifications" 체크

##### Android — google-services.json
- [ ] Firebase Console → Project settings → General → "Add app" → Android (이미 있으면 skip)
  - Package name: `com.uihyun.drape`
- [ ] `google-services.json` 다운로드 → `android/app/google-services.json` 에 놓기
- [ ] `android/build.gradle` 의 buildscript dependencies 에 `classpath 'com.google.gms:google-services:4.4.2'` 추가되어 있는지 확인 (Capacitor 7 기본 포함)
- [ ] `android/app/build.gradle` 맨 아래에 `apply plugin: 'com.google.gms.google-services'` 추가

##### 네이티브 sync + 첫 테스트
```bash
npx cap sync ios
npx cap sync android
npx cap open ios   # Xcode 빌드 → 실기기 (시뮬레이터 푸시 불가)
```
- [ ] iOS 실기기에서 앱 첫 실행 → 푸시 권한 요청 다이얼로그 → "허용"
- [ ] 다른 계정에서 DM 전송 → 잠금화면에 푸시 도착 확인
- [ ] 푸시 탭 → `/messages/{tid}` 로 딥링크 동작 확인

#### 다음에 추가할 트리거 (현재 DM 만)

코드 자리는 잡혀 있으니 같은 패턴으로 추가 가능:
- 좋아요 알림 (`outfits/{id}` likedBy 변경 트리거)
- 팔로우 알림 (`follows/{id}` 생성 트리거)
- 주간 다이제스트 (pubsub 스케줄)
- 일일 OOTD 리마인더 (24h 미접속자)
